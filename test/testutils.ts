...
import {
  ...
  SimpleAccountInP256,
  SimpleAccountInP256Factory__factory,
  SimpleAccountInP256__factory,
  SimpleAccountInP256Factory,
...
): Promise<{ actualGasCost: BigNumber }> {
...
export async function createAccountInP256(
  ...
  const proxy = SimpleAccountInP256__factory.connect(
    accountAddress,
    ethersSigner
  );
  return {
...
export function unpackAccountGasFees(accountGasFees: string): {
  maxPriorityFeePerGas: number;
  maxFeePerGas: number;
} {
  return {
    maxPriorityFeePerGas: parseInt(accountGasFees.slice(2, 34), 16),
    maxFeePerGas: parseInt(accountGasFees.slice(34), 16),
  };
}
...
export async function callGetUserOpHashWithCode(
  entryPoint: EntryPoint,
  ...
  return await ethers.provider.send("eth_call", [
    {
      ...
      data: entryPoint.interface.encodeFunctionData("getUserOpHash", [
        packUserOp(userop),
      ]),
    },
    "latest",
